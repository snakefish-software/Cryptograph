...
class FourSquare (val plainSquare: PolybiusSquare,
  ...
  def decrypt(ciphertext: CharSequence): String = {
    ...
    val inSquareLength = inSquareChars.length
    if (strictMode && inSquareLength % 2 == 1)
      throw new OddCiphertextLengthException()

    var plaintext = new StringBuilder(inSquareLength)
    for (i <- 0 until inSquareLength by 2) {
      if (i + 1 < inSquareLength) {
